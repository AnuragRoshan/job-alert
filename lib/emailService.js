...
const getBaseUrl = () => {
  ...
  // Custom production domain or explicit NEXTAUTH_URL
...
const createTransporter = () => {
  ...
  return nodemailer.createTransporter(emailServices[serviceType]);
};
...
export const sendJobAlert = async (email, jobs, company) => {
    ...
    html: `
      ...
      jobs.length > 1 ? "s" : ""
          ...
          <div style="text-align: center; margin: 24px 0; padding: 20px; background: #f8fafc; border-radius: 8px;">
            <p style="color: #475569; margin-bottom: 16px; font-weight: 600;">Want to manage your alerts?</p>
  ...
  try {
    ...
    console.log(`Job alert email sent to ${email} for ${company}`);
    return { success: true };
  } catch (error) {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #10b981, #3b82f6); padding: 24px; text-align: center; border-radius: 8px 8px 0 0;">
          <h1 style="color: white; margin: 0;">🎉 Alert Activated!</h1>
          <p style="color: #d1fae5; margin: 8px 0 0 0;">You're all set to catch new opportunities</p>
        </div>
        
        <div style="background: white; padding: 24px; border-radius: 0 0 8px 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
          <p style="color: #374151; font-size: 16px; margin-bottom: 20px;">
            Hi there! 👋 Your job alert for <strong>${company}</strong> is now active.
          </p>
